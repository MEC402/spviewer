...
// 	  for (int j = 0; j < cols; j++){
// 	    
// 	    lsphere[j + i * cols] = createGeode(newurl, i, j, rows, cols, panos[0]->getLeftImages(), 0x01);
// 	    leftrotate[j + i * cols] = new osg::PositionAttitudeTransform;
...
// 	  //  double d=rotatedLeftSphere[0];
// 	   // double d1=verticalRotation[0];
//     	    //std::cout<<d<<std::endl;
// 
// 	    //while(d<0.0001){
...
// 	     		  	     pat->setAttitude(myQuat[0]);
// 
// 	    rsphere[j + i * cols] = createGeode(newurl, i, j, rows, cols, panos[0]->getRightImages(), 0x02);
...
// 
// 	int a= getIndex();
// 	      setIndex((a+getTotalImages()-1)%panos.size());
// 	      a=getIndex();
// 
// 	if (node != NULL){
// 	
